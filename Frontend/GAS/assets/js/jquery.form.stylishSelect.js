  ...
  $.fn.sSelect = function (options) {
    return this.each(function () {
      ...
      if ($input.attr("style") != undefined) {
        opts.containerStyle = $input.attr("style");
      } //custom 2012-09-27 @wddo : copy style
      var $containerDivText = $(
          '<div title="' +
            $input.attr("title") +
            '" class="selected-headline" style="' +
            opts.containerStyle +
            '"></div>'
        ), //custom 2012-09-27 @wddo : copy style
        $containerDiv = $(
        ...
        $newUl = $(
          '<ul class="select-list" style="visibility:hidden;' +
            opts.containerStyle +
            '"></ul>'
        ),
        itemIndex = -1,
      ...
      if ($input.children("optgroup").length == 0) {
        $input.children().each(function (i) {
          ...
          $newUl.append(
            $('<li><a href="JavaScript:void(0);">' + option + "</a></li>").data(
              "key",
              key
            )
          );
        ...
        $input.children("optgroup").each(function () {
          var optionTitle = $(this).attr("label"),
            $optGroup = $(
              '<li class="select-list-option-title">' + optionTitle + "</li>"
            );
      ...
      if (currentIndex != -1) {
        ...
        $containerDivText.html(
          '<span style="margin:0 0 0 10px;">' + opts.defaultText + "</span>"
        );
      }
      ...
      $newLi
        .bind("mouseenter.sSelect", function (e) {
      ...
      function navigateList(currentIndex, init) {
        ...
        $newLi
          .removeClass("select-list-selected")
          .eq(currentIndex)
        ...
        if (
          $input.find("> option").eq(currentIndex).attr("value").slice(0, 7) ===
          "http://"
        ) {
          $input
            .siblings("a")
            .attr(
              "href",
              $input.find("> option").eq(currentIndex).attr("value")
            );
        }
      ...
      function keyPress(element) {
        ...
        $(element)
          ...
          .bind("keydown.sSelect", function (e) {
            var keycode = e.which;
            ...
            if (typeof currentKeyIndex != "undefined") {
              ...
              if (
                ...
                currentIndex = keys.indexOf(keyPressed); //if no entry was found or new key pressed search from start of array
      ...
      $containerDiv.bind("focus.sSelect mouseover.sSelect", function () {
        //custom 2013-03-12 @wddo : xhtml by add mouseover
        if (getDisabled()) return;
        if (typeof document.attachEvent === "undefined")
          $containerDivText.addClass("select-list-selfocus"); //custom 2012-10-18 @wddo : ie7 list scrolling error
        keyPress(this);
      });

      $containerDiv.bind("blur.sSelect mouseout.sSelect", function () {
        //custom 2013-03-12 @wddo : xhtml by add mouseout
        if (getDisabled()) return;
        if (typeof document.attachEvent === "undefined")
          $containerDivText.removeClass("select-list-selfocus"); //custom 2012-10-18 @wddo : ie7 list scrolling error
      });
